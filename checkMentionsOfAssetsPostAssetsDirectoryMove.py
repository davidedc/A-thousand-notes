...
for eachDirectory in attachmentsDirectoryNames:
    ...
    plainReferencesToDirectory = unicode("![](assets/" + eachDirectory_bearEscaped + "/")
    ...
    for noteFileName in notesFileNames:
        ...
        with codecs.open(noteFilePath, 'r', encoding='utf-8') as file:
            ...
            complexMarkdownOccurrences_re = re.compile('\\[[^\\]]*\\]\\(' + re.escape(ASSETS_ABSOLUTE_PATH.lower() + eachDirectory_bearEscaped_lower + "/"))
            ...
            complexMarkdownOccurrences_re = re.compile('\\[[^\\]]*\\]\\(' + re.escape("assets/" + eachDirectory_bearEscaped_lower + "/"))
            #print(complexMarkdownOccurrences_re.pattern)
            #print(data_lower)
            if re.search(complexMarkdownOccurrences_re, data_lower):
                #print(" " + eachDirectory + " found (also) in form [something](directory) in note " + noteFileName)
        ...
        for assetFile in assetsFiles:
            ...
            for noteFileName in notesFileNames:
                ...
                with codecs.open(noteFilePath, 'r', encoding='utf-8') as file:
                    ...
                    # ---------------------------------------------------------
                    plainReferencesToAsset = re.compile(re.escape("![](assets/" + eachDirectory_bearEscaped_lower + "/" + assetFile_bearEscaped_lower + ")"))
                    ...
                    # ---------------------------------------------------------
                    complexMarkdownOccurrences_re = re.compile('\\[[^\\]]*\\]\\(' + re.escape(ASSETS_ABSOLUTE_PATH.lower() + eachDirectory_bearEscaped_lower + "/" + assetFile_bearEscaped_lower + ")"))
                    #print(complexMarkdownOccurrences_re.pattern)
                    #print(data_lower)
                    if re.search(complexMarkdownOccurrences_re, data_lower):
                        #print(" " + assetFile + " found (also) in form [something](directory) in note " + noteFileName)
                    ...
                    # ---------------------------------------------------------
                    markdownLinkToDirectory_re = re.compile(re.escape("[" + assetFile_lower + "](" + ASSETS_ABSOLUTE_PATH.lower() + eachDirectory_bearEscaped_lower + "/)" ))
